...
- **心理的負担・プライバシー侵害感の解消**: 一方的に監視されているような窮屈さやプレッシャーを感じさせることなく、日常の自然なスマホ利用（画面点灯など）を通じてさりげなくお互いの無事を確認し合えます。

---

## 🔒 セキュリティ & プライバシー

1. **SkyWayによる堅牢な通信プロトコル**:
   - NTT Communicationsが提供する信頼性の高いWebRTCプラットフォーム「SkyWay」を採用。強固に暗号化されたプロトコル（DTLS-SRTP / SCTP over DTLS）により、通信経路上での盗聴や改ざんを防止します。
2. **通信内容は「本人の状態」のみに限定**:
   - 送信されるデータは「画面点灯（活動シグナル）」や「元気／良くない」といった本人の状態ステータス情報のみです。
   - 位置情報（GPS）、カメラ映像、音声、端末内の個人情報や操作ログなどは一切取得・送信しません。
3. **サーバーにデータを保有しない完全P2P設計**:
   - 中央サーバーにメッセージや利用履歴などのプライベートデータを一切蓄積・保有しないP2P（Peer-to-Peer）通信方式を採用しています。
   - 万が一外部への情報漏洩リスクが生じた場合でも、サーバー上に個人の安否履歴データ自体が存在しないため、プライバシーが強固に保護されます。
